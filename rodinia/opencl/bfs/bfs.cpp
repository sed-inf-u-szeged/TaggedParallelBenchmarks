...
void run_bfs_gpu(int no_of_nodes, Node *h_graph_nodes, int edge_list_size, \
		int *h_graph_edges, char *h_graph_mask, char *h_updating_graph_mask, \
		char *h_graph_visited, int *h_cost,  int deviceNumber) 
					throw(std::string){
	...
	try{
		...
		DYNAMIC_BEGIN("BFS_initialize_cl_objects")
		_clInit(deviceNumber);
		...
		DYNAMIC_BEGIN("BFS_transfer_data_from_host_to_device")
		...
		//--2 invoke kernel
#ifdef	PROFILING
		DYNAMIC_BEGIN("BFS_kernel_profiling")
		timer kernel_timer;
		...
		do{
			...
			int kernel_id = 0;
			int kernel_idx = 0;
			DYNAMIC_BEGIN("BFS_kernel_1")
			STATIC_BEGIN("BFS_kernel_1", (no_of_nodes, h_graph_mask, h_graph_nodes, h_graph_edges, h_graph_visited, h_cost, h_updating_graph_mask), (h_cost, h_updating_graph_mask))
			...
			//--kernel 1
			DYNAMIC_BEGIN("BFS_kernel_2")
			STATIC_BEGIN("BFS_kernel_2", (no_of_nodes, h_updating_graph_mask, h_graph_mask, h_graph_visited, stop), (h_updating_graph_mask, h_graph_mask, h_graph_visited, stop))
			kernel_id = 1;
			kernel_idx = 0;
			_clSetArgs(kernel_id, kernel_idx++, d_graph_mask);
			_clSetArgs(kernel_id, kernel_idx++, d_updating_graph_mask);
			...
			DYNAMIC_BEGIN("BFS_transfer_data_from_device_to_host")
		...
		DYNAMIC_BEGIN("BFS_release_cl_resources")
...
fprintf(stderr,"Usage: %s <input_file> <DeviceNo. [CPU: 1, GPU: 0]>\n", argv[0]);
...
int main(int argc, char * argv[])
{
	...
	try{
		...
		//printf("ide eljut\n");
				//---------------------------------------------------------
		//--gpu entry
		run_bfs_gpu(no_of_nodes,h_graph_nodes,edge_list_size,h_graph_edges, h_graph_mask, h_updating_graph_mask, h_graph_visited, h_cost, deviceNumber);
		//---------------------------------------------------------
		//--cpu entry
