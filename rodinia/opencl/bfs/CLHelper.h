...
void _clInit(int deviceNumber)
{
//strcpy(kernelFileName,"./kernel00.clbin");
platformAndDevice(plat,dev,&platDev);
		printf("Platform:%d, Device:%d\n",platDev.platformId,platDev.deviceId);
//platDev.platformId = 0;
//platDev.deviceId = 0;
FILE * fp = fopen(kernelFileName, "rb"); 
	if(!fp) { printf("ERROR: unable to open '%s'\n", kernelFileName); return -1; }
    ...
    targetPlatform = allPlatforms[platDev.platformId];
   ...
   oclHandles.cl_status = clGetDeviceIDs(targetPlatform, device_type, deviceListSize, \
								oclHandles.devices, NULL);
   cl_device_id device_id = oclHandles.devices[platDev.deviceId];
   if(oclHandles.cl_status!=CL_SUCCESS){
   	throw(string("exception in _clInit -> clGetDeviceIDs-2"));   	
   }

    oclHandles.context = clCreateContext(0,1,&device_id,NULL,NULL,&resultCL);
   ...
   //--cambine-4: Create an OpenCL command queue    
    ...
    /*oclHandles.program = clCreateProgramWithSource(oclHandles.context, 
                                                    ...
                                                    sourceSize,
                                                    &resultCL);*/
    oclHandles.program = clCreateProgramWithBinary(oclHandles.context,1,&device_id,&content_length, &content_const, 0,&resultCL);
    ...
    resultCL = clBuildProgram(oclHandles.program, 0, NULL,NULL,NULL,NULL);
