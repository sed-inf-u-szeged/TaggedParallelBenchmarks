...
int main(int argc, char *argv[]) {
	...
	DYNAMIC_BEGIN("NN_initialize_cl_objects")
	platformAndDevice(argv[9],argv[10],&platDev);
	printf("Platform:%d, Device:%d\n",platDev.platformId,platDev.deviceId);
	//context = cl_init_context(platform,device,quiet);
	context = cl_init_context(platDev.platformId,platDev.deviceId,quiet);
	DYNAMIC_END

	recordDistances = OpenClFindNearestNeighbors(context,numRecords,locations,lat,lng,timing,argv);
...
float *OpenClFindNearestNeighbors(
	...
	DYNAMIC_BEGIN("NN_initialize_cl_objects")
			cl_kernel NN_kernel;
				...
				cl_NN_program = cl_compileProgram(argv[8],NULL);
		...
		DYNAMIC_BEGIN("NN_transfer_data_from_host_to_device")
		...
		DYNAMIC_BEGIN("NN_kernel")
		STATIC_BEGIN("NN_kernel", (rec_count, sandbox, z, target_lat, target_long), (z))
		cl_int argchk;
		...
		DYNAMIC_BEGIN("NN_transfer_data_from_device_to_host");
	...
	DYNAMIC_END
		...
		DYNAMIC_BEGIN("NN_release_cl_resources");
