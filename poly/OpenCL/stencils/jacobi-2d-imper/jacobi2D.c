...
int main(int argc, char *argv[])
{
	...
	DYNAMIC_BEGIN("JACOBI2D_init")
	read_cl_file(argv[3]);
	cl_initialization(platDev.platformId, platDev.deviceId);
	DYNAMIC_END
	
	DYNAMIC_BEGIN("JACOBI2D_xfer_to")
	...
	for (t = 0; t < _PB_TSTEPS ; t++)
    	{
		DYNAMIC_BEGIN("JACOBI2D_kernel_1")
		cl_launch_kernel1(n);
		DYNAMIC_END
		
		DYNAMIC_BEGIN("JACOBI2D_kernel_2")
		cl_launch_kernel2(n);
		DYNAMIC_END
	}
	...
	DYNAMIC_BEGIN("JACOBI2D_xfer_from")
	...
	DYNAMIC_BEGIN("JACOBI2D_cleanup")
	cl_clean_up();
	DYNAMIC_END
...
void runJacobi2D_kernel1(DATA_TYPE* A, DATA_TYPE* B, int n)
{
	STATIC_BEGIN("JACOBI2D_KERNEL_1", (), ())
...
void runJacobi2D_kernel2(DATA_TYPE* A, DATA_TYPE* B, int n)
{
	STATIC_BEGIN("JACOBI2D_KERNEL_2", (), ())
	int i = get_global_id(1);
	int j = get_global_id(0);
